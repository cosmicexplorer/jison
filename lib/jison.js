...
var NonterminalForest = Jison.NonterminalForest = class NonterminalForest {
    ...
    pushProductionForSymbol(symbol, production) {
        this.bySymbol.get(symbol).productions.push(production);
    }
...
generator.compileNextProduction = function compileNextProduction(handle, symbols) {
    console.dir({handle, h: handle.length, i: typeof handle[1]});
    ...
    if (!Array.isArray(handle)) {
        ...
        for (const symString of rhs) {
            if (symString === 'error')
                her = true;
            symbols.tryRegisterNewSymbol(symString);
        }
        return r = new Production(symbol, rhs, productions.length+1);
    }
    ...
    if (handle.length < 3) {
        ...
        // no action -> don't care about aliases; strip them.
        const stripped = rhs.map(function(e,i) { return e.replace(/\[[a-zA-Z_][a-zA-Z0-9_-]*\]/g, '') });
        // only precedence specified
        r = new Production(symbol, stripped, productions.length+1);
        const operator = handle[1];
        if (operators[operator.prec]) {
            r.precedence = operators[operator.prec].precedence;
        }
        return r;
    }

    {
            ...
            let action = handle[1];
            console.dir({label, action});

            // replace named semantic values ($nonterminal)
            if (action.match(/[$@][a-zA-Z][a-zA-Z0-9_]*/)) {
                const count = new Map;
                const names = new Map;
                for (const i = 0; i < rhs.length; i++) {
                    const symString = rhs[i];
            ...
            const operator = handle[2];
            if (operator && operators[operator.prec]) {
                r.precedence = operators[operator.prec].precedence;
            }
        }
    ...
    if (r.precedence === 0) {
        ...
        for (let i = r.handle.length-1 ; i>=0; i--) {
            ...
            if (!(nonterminals.hasSeenThisSymbol(curOp)) && (curOp in operators)) {
                r.precedence = operators[curOp].precedence;
            }
        }
    }
    ...
    productions.push(r);
    productions_.push([
        symbols.getIdRegisteredForSymbol(r.symbol),
        // TODO: absolutely no clue what either of these are for or otherwise signify.
        r.handle[0] === '' ? 0 : r.handle.length,
    ]);
    nonterminals.pushProductionForSymbol(symbol, r);


    if (Array.isArray(handle)) {
        // if (handle.constructor === Array) {
        // const [rhs, action, operator] = handle;
        const rhs = (typeof handle[0] === 'string') ?
              handle[0].trim().split(' ') :
              handle[0].slice(0);
...
generator.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators) {
    ...
    const actionGroups = new ActionGroups();
    var productions_ = [0];
    ...
    for (const symbol of Object.getOwnPropertyNames(bnf)) {
        ...
        prods.forEach((handle) => this.compileNextProduction(handle, symbols));
    }

    // TODO: we could possibly just push all the values together at once, but this is how the
    // previous code did it.
    for (const [joined, base, breakCase] of actionGroups.iterAllRecordedActions()) {
...
lrGeneratorMixin.closureOperation = function closureOperation (itemSet /*, closureSet*/) {
    var closureSet = new this.ExtendedItemSet();
    var self = this;

    var set = itemSet,
        itemQueue, syms = new Set;
    console.dir({closureSet, self, set});

    do {
        // NB: previously was just Set, not ItemSet?
        itemQueue = new this.ExtendedItemSet();
        closureSet.concat(set);
        set.forEach(function CO_set_forEach (item) {
            var symbol = item.markedSymbol;
            console.dir({symbol, item, nt: self.nonterminals[symbol]});
...
lrGeneratorMixin.canonicalCollection = function canonicalCollection () {
    console.dir({p: this.productions});
    var item1 = new this.Item(this.productions[0], 0, [this.EOF]);
    console.dir({p: this.productions.slice(0, 5), pi: item1});
    var firstState = this.closureOperation(new this.ExtendedItemSet([item1])),
        states = new this.ExtendedItemSet([firstState]),
        marked = 0,
        self = this,
        itemSet;

    // FIXME: new class for this!
    states.has = {};
    states.has[firstState] = 0;

    console.dir({
        states: Array.from(states[Symbol.iterator]().take(5)),
        firstState,
        item1,
    }, { depth: 3 });
    while (marked !== states.size()) {
        // console.dir({marked, s: states.size()}, { depth: null });
        itemSet = states.item(marked); marked++;
        console.dir({itemSet}, { depth: 2 });
