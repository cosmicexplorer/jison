...
lookaheadMixin.followSets = function followSets () {
    ...
    // const nontermKeys = Object.keys(nonterminals).slice(0, 15);
    // const nontermValues = nontermKeys.map((k) => nonterminals[k]);
    // console.dir({
    //     productions: {
    //         excerpt: productions.slice(0, 15),
    //         len: productions.length,
    //     },
    //     nonterminals: {
    //         excerpt: nontermValues,
    //         len: nontermKeys.length,
    //     },
    // }, { depth: null });
...
lrGeneratorMixin.ItemSet = class ItemSet {
    ...
    computeStableDelimitedKeyString() {
        ...
        // NB: the original code uses this._items.map(...); this is equivalent and may be faster.
    }
    valueOf() {
...
lrGeneratorMixin.closureOperation = function closureOperation (itemSet /*, closureSet*/) {
    ...
    set.forEach(function CO_set_forEach (item) {
        ...
        // var ntKeys = Object.keys(self.nonterminals);
        // var ntVals = {}
        // ntKeys.slice(0, 5).forEach(function (k) { ntVals[k] = self.nonterminals[k]; });
        // console.dir({syms, nt: {len: ntKeys.length, excerpt: ntVals}}, { depth: 3 });
        // console.dir({
        //     closureSet: {
        //         len: closureSet._items.length,
        //         excerpt: closureSet._items.slice(0, 4),
        //     },
        //     itemQueue: {
        //         len: itemQueue._items.length,
        //         excerpt: itemQueue._items.slice(0, 4),
        //     },
        // }, { depth: null });
...
lrGeneratorMixin.canonicalCollection = function canonicalCollection () {
    ...
    var firstState = this.closureOperation(new this.ItemSet(item1)),
        states = new this.ItemSet([firstState]),
        marked = 0,
        self = this,
        itemSet;

    // console.dir({states, ss: states.size(), item1}, { depth: 3 });
    // console.dir({
    //     p: {
    //         len: this.productions.length,
    //         excerpt: this.productions.slice(0, 3),
    //     },
    //     EOF: this.EOF,
    //     firstState,
    // }, { depth: null });
    ...
    unionLookaheads: function LALR_unionLookaheads () {
        ...
        states.forEach(function union_states_forEach (i) {
            ...
            state.reductions.forEach(function union_reduction_forEach (item) {
                ...
                // console.dir({item, j: item.production.handle.join(' ')}, { depth: null });
