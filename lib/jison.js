    ...
    while(cont) {
        ...
        productions.forEach(function Follow_prod_forEach (production, k) {
            ...
            for (var i=0,t;t=production.handle[i];++i) {
                ...
                var bool = !ctx || q === parseInt(self.nterms_.get(t), 10);
                ...
                if (i === production.handle.length+1 && bool) {
                    ...
                    if (self.nullable(part) && bool) {
                        set = set.union(pSym.follows);
                    }
                }
                const oldcount = curNonterminal.follows.size;
                // (2)FIXME: this is so slow!
                curNonterminal.follows = curNonterminal.follows.union(set);
...
lookaheadMixin.first = function first (symbol) {
    ...
    // RHS
        ...
        for (var i=0,t;t=symbol[i];++i) {
            if (!this.nonterminals.hasSeenThisSymbol(t)) {
                firsts.add(t);
            } else {
                const rFirst = this.nonterminals.getBySymbol(t).first;
                firsts = firsts.union(rFirst);
            }
            if (!this.nullable(t))
                break;
        }
