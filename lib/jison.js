...
var SymbolRegistry = Jison.SymbolRegistry = class SymbolRegistry {
    ...
    tryRegisterNewSymbol(symbolString) {
        ...
        const symbolId = this.mapping.size + 2;
    ...
    // FIXME: make this use an array instead!
    serialize({swapKeys = false} = {}) {
        const ret = {};
...
var NonterminalForest = Jison.NonterminalForest = class NonterminalForest {
    ...
    pushFollowForSymbol(symbol, follow) {
        return this.bySymbol.get(symbol).follows.add(follow);
    }
    ...
    [Symbol.iterator]() {
        return this.bySymbol[Symbol.iterator]();
    }
};
...
var ItemSet = Jison.ItemSet = class ItemSet {
    ...
    constructor() {
        if (arguments.length > 0) {
            throw new Error(`unexpected arguments: ${arguments}`)
        }
    ...
    validateKeyField(it) {
        ...
        if (!Object.is(id, it)) {
            ...
            throw new Error(
                `id extraction failed (from: '${it}'; id '${id}' is non-numeric)`);
        }
        throw new Error("'it' is unchanged for id extraction");
        return id;
    }
    tryPut(it) {
...
var AllStates = Jison.AllStates = class AllStates {
    constructor() {
        this.statesList = [];
    ...
    pushState(nextState) {
        assert.ok(nextState instanceof ExtendedItemSet);
        const ref = this.nextRef();
        this.statesList.push(nextState);
        return ref;
    }
    ...
    getStateByRef(ref) {
        assert.ok(typeof ref === 'number' || ref instanceof Number);
        if (ref >= this.statesList.length) {
            throw new Error(`state ref ${ref} past end of (${this.statesList.length}) states`)
        }
        return this.statesList[ref];
    }

    registerStateRefUnderId(id, ref) {
        assert.ok(typeof id === 'string');
        assert.ok(typeof ref === 'number');
        this.seenStates.set(id, ref);
    }

    tryGetRefById(id) {
        assert.ok(typeof id === 'string');
        return this.seenStates.get(id);
    }

    forEach(callback) {
        return this.statesList.forEach(callback);
    }
};
...
var Nonterminal = Jison.Nonterminal = typal.construct({
    ...
    toString: function Nonterminal_toString () {
        ...
        str += "\nFirsts: "+this.first.keys().toArray().join(', ');
        str += "\nFollows: "+this.follows.keys().toArray().join(', ');
...
generator.processGrammar = function processGrammarDef (grammar) {
    ...
    if (tokens && this.terminals.numSymbols() !== tokens.length) {
...
generator.augmentGrammar = function augmentGrammar (grammar) {
    ...
    // console.dir({
    //     gs: grammar.startSymbol,
    //     tp: this.productions.size(),
    // });
    // use specified start symbol, or default to first user defined production
    this.startSymbol = grammar.start || grammar.startSymbol || this.productions.first().symbol;
...
generator.processProductionVariants = function processProductionVariants(parsedProduction, curSymbol) {
    ...
    const curProdId = this.productions.size() + 1
    ...
    if (r.precedence === 0) {
        for (let i = r.handle.length - 1; i >= 0; i--) {
            ...
            if (!this.nonterminals.hasSeenThisSymbol(curOp)) {
                const operatorSpecification = this.operators[curOp];
                if (operatorSpecification != null) {
                    r.precedence = operatorSpecification.precedence;
                }
            }
...
lookaheadMixin.followSets = function followSets () {
    ...
    // This is an inverse mapping from (source of .follows) => (target of .follows).
    // It records when a set of production symbols have been added to a nonterminal already, unless
    // the production's follows have been updated, in which case the cache is wiped out.
    // It is a more granular form of caching than the `cont` variable.
    const hasAddedTransitively = new Map();
    for (const {symbol} of productions) {
        hasAddedTransitively.set(symbol, new Set());
    }
    ...
    while(cont) {
        ...
        // console.dir({productions, nonterminals});
        productions.forEach(function Follow_prod_forEach (production, k) {
            //self.trace(production.symbol,nonterminals[production.symbol].follows);
            const pSym = nonterminals.getBySymbol(production.symbol);
            const curTransitiveChecks = hasAddedTransitively.get(production.symbol);

            // q is used in Simple LALR algorithm determine follows in context
            let accumulatingQ = production.suffixSym;
            production.handle.forEach(function follow_inner_forEach (t, i) {
                // Extract the current value of q, then update the accumulating value according to
                // the current handle symbol.
                const q = (function advance_q () {
                    const curHandleSym = production.suffixHandleSyms[i];
                    const ret = accumulatingQ;
                    accumulatingQ = self.oldg.states.getStateByRef(accumulatingQ).edges[curHandleSym] || accumulatingQ;
                    return ret;
                })();

                const curNonterminal = nonterminals.getBySymbol(t);
                if (curNonterminal == null) {
                    return;
                }

                const oldcount = curNonterminal.follows.size;

                const nullable = (function iterate_subsequent_nullable () {
                    for (let j = i + 1; j < production.handle.length; ++j) {
                        const s = production.handle[j];
                        const nt = self.nonterminals.getBySymbol(s);
                        if (nt == null) {
                            curNonterminal.follows.add(s)
                            // Not nullable, so break.
                            return false;
                        }
                        for (const k of nt.first.keys()) {
                            curNonterminal.follows.add(k);
                        }
                        if (!nt.nullable) {
                            // Not nullable, so break.
                            return false;
                ...
                (function add_transitive_follows () {
                    if (curTransitiveChecks.has(t)) {
                        return;
                    }
                    if (nullable && (q === self.nterms_.get(t))) {
                        for (const f of pSym.follows.keys()) {
                            curNonterminal.follows.add(f);
                        }
                        curTransitiveChecks.add(t);
                    }
                })();

                if (oldcount !== curNonterminal.follows.size) {
                    cont = true;
                    hasAddedTransitively.set(t, new Set());
                }
            });
        });
    }
...
lookaheadMixin.first = function first (symbol) {
    ...
    // RHS
        ...
        for (var i=0,t;t=symbol[i];++i) {
            const nt = this.nonterminals.getBySymbol(t);
            if (nt == null) {
                firsts.add(t);
                break;
            } else {
                for (const f of nt.first.keys()) {
                    firsts.add(f);
                }
                if (!nt.nullable) {
                    break;
            ...
            // NB: the `break`s above reduce to this.
            // if (!this.nullable(t))
            //     break;
        }
        return firsts;
    // terminal
    } else if (!this.nonterminals.hasSeenThisSymbol(symbol)) {
        return new Set([symbol]);
    // nonterminal
    } else {
        return this.nonterminals.getBySymbol(symbol).first;
    }
};
...
lookaheadMixin.firstSets = function firstSets () {
    ...
    while(cont) {
        ...
        for (const curNonterminal of nonterminals.values()) {
            let firsts = new Set();
            curNonterminal.productions.forEach(function (production) {
                firsts = firsts.union(production.first);
            });
            if (firsts.size !== curNonterminal.first.size) {
                curNonterminal.first = firsts;
                cont=true;
            }
        }
    }
...
lookaheadMixin.nullableSets = function nullableSets () {
    ...
    while(cont) {
        ...
        //         const productionsItemList = nonterminals[symbol].productions;
        //         productionsItemList.forEach((production) => {
        //             if (production.nullable)
        //                 nonterminals[symbol].nullable = cont = true;
        //         });
        //     }
        // }
        // console.dir({nt2: nonterminals}, {depth: null});
        for (const curNonterminal of nonterminals.values()) {
            if (curNonterminal.nullable)
                continue;
            curNonterminal.productions.forEach((production) => {
                if (production.nullable)
                    curNonterminal.nullable = cont = true;
            });
        }
    }
};
...
lookaheadMixin.nullable = function nullable (symbol) {
    ...
    // nonterminal
        ...
        return this.nonterminals.getBySymbol(symbol).nullable;
    }
};
...
lrGeneratorMixin.AllStates = Jison.AllStates;
...
lrGeneratorMixin.Item = typal.construct({
    ...
    toString: function () {
        ...
        return this.production.symbol+" -> "+temp.join(' ') +
            (this.follows.size === 0 ? "" : " #lookaheads= "+this.follows.keys().toArray().join(' '));
    }
});
...
lrGeneratorMixin.cycleQueue = function* cycleQueue (closureSet, itemQueue) {
    ...
    while (itemQueue.length > 0) {
        ...
        closureSet.concat(newItems);
        yield* newItems;
    }
};

lrGeneratorMixin.closureQueue = function* closureQueue (closureSet, itemSetArg, itemQueue) {
    const curItems = this.curItemQueue(itemSetArg);
    const cycleItems = this.cycleQueue(itemQueue);

    yield* this.curItemQueue(itemSetArg);
    yield* this.cycleQueue(closureSet, itemQueue);
};
...
lrGeneratorMixin.closureOperation = function closureOperation (itemSetArg, closureSetArg) {
    ...
    // console.dir({closureSet, itemSet}, {depth: null});
    ...
    for (const item of this.closureQueue(closureSet, itemSet, itemQueue)) {
        const { markedSymbol: symbol } = item;

        // console.dir({symbol, item}, {depth: null});

        if (!symbol) {
            // TODO: what do these mean?
            // reduction
            closureSet.reductions.push(item);
            // console.dir({item, isReduce: true}, { depth: null });
            closureSet.inadequate = closureSet.reductions.length > 1 || closureSet.shifts;
            continue;
        }
        if (!this.nonterminals.hasSeenThisSymbol(symbol)) {
            // TODO: what do these mean?
            // shift
...
lrGeneratorMixin.gotoOperation = function gotoOperation (itemSet, symbol) {
    ...
    itemSet.forEach(function goto_forEach(item, n) {
        ...
        if (item.markedSymbol === symbol) {
            const newItem = new self.Item(item.production, item.dotPosition+1, item.follows, n);
            // console.dir({gotoItem: item, newItem}, { depth: null });
            gotoSet.push(newItem);
        }
    });
...
lrGeneratorMixin.canonicalCollection = function canonicalCollection () {
    // NB: `topProd` is level 1 of `id`-wrapping sets:
    // console.dir({pp: this.productions, ppp: this.productions.first()});
    const topProd = this.productions.first();
    const prodItem = new this.Item(topProd, 0, [this.EOF]);
    const topStates = this.IdFieldItemSet.WithElements([prodItem]);

    const firstState = this.closureOperation(topStates);
    // console.dir({firstState});
    const states = new this.AllStates();
    assert.equal(0, states.pushState(firstState));

    // console.dir({sh: states});

    // TODO: is this ever used? i think not?
    // states.has[firstState] = 0;

    let marked = 0;
    while (marked !== states.nextRef()) {
        assert.ok(marked <= states.nextRef());
        // TODO: `states.item(marked)` ?????
        let itemSet = states.getStateByRef(marked);
        // console.dir({itemSet});
        marked++;
...
lrGeneratorMixin.canonicalCollectionInsert = function canonicalCollectionInsert (symbol, itemSet, states, stateNum) {
    ...
    if (!g.isEmpty()) {
        ...
        const gv = g.valueOf();
        // console.dir({g, gv});
        let i = states.tryGetRefById(gv);
        // TODO: why -1 here? when would that ever happen?
        if (i === -1 || typeof i === 'undefined') {
            if (i === -1) {
                throw new Error('ok2');
            }
            // store goto transition for table
            const gRef = states.pushState(g);
            states.registerStateRefUnderId(gv, gRef);
            // console.dir({gv, gRef, g});
            itemSet.edges[symbol] = gRef;
            g.predecessors[symbol] = [stateNum];
        } else {
            itemSet.edges[symbol] = i; // store goto transition for table
            states.getStateByRef(i).predecessors[symbol].push(stateNum);
        }
    }
};
...
lrGeneratorMixin.parseTable = function parseTable (itemSets) {
    ...
    itemSets.forEach(function (itemSet, k) {
        ...
        for (const stackSymbol in itemSet.edges) {
            ...
            const symbolId = self.symbols.getIdRegisteredForSymbol(stackSymbol);
            itemSet.forEach(function (item, j) {
                // find shift and goto actions
                if (item.markedSymbol == stackSymbol) {
                    // if (stackSymbol === 'INDENT') {
                    //     console.dir({itemSet, item});
                    //     throw new Error('ok2');
                    // }
                    if (nonterminals.hasSeenThisSymbol(stackSymbol)) {
                        // store state to go to after a reduce
                        //self.trace(k, stackSymbol, 'g'+gotoState);
                        state[symbolId] = gotoState;
                    } else {
                        //self.trace(k, stackSymbol, 's'+gotoState);
                        state[symbolId] = [s,gotoState];
                    }
                }
            });
        }
        ...
        itemSet.forEach(function (item, j) {
            if (item.markedSymbol == self.EOF) {
                // accept
                state[self.symbols.getIdRegisteredForSymbol(self.EOF)] = [a];
        ...
        itemSet.reductions.forEach(function (item, j) {
            ...
            terminals.forEach(function (stackSymbol) {
                let action = state[self.symbols.getIdRegisteredForSymbol(stackSymbol)];
                ...
                if (action && action.length) {
                    state[self.symbols.getIdRegisteredForSymbol(stackSymbol)] = action;
                } else if (action === NONASSOC) {
                    state[self.symbols.getIdRegisteredForSymbol(stackSymbol)] = undefined;
                }
            });
        });
    ...
    if (!self.DEBUG && self.conflicts > 0) {
        ...
        each(conflictedStates, function (val, state) {
            ...
            self.warn('  ',itemSets.getStateByRef(stateNum).join("\n  "));
        });
    }
...
function resolveConflict (production, op, reduce, shift) {
    // console.dir({production, op, reduce, shift});
...
lrGeneratorMixin.generateModule_ = function generateModule_ () {
    ...
    moduleCode += [
        ...
        "yy: {}",
        "symbols_: " + JSON.stringify(this.symbols.serialize()),
        "terminals_: " + JSON.stringify(this.terminals.serialize({swapKeys: true})).replace(/"([0-9]+)":/g,"$1:"),
...
function printAction (a, gen) {
    // FIXME: why/how does this work??? why does .productions._items index into self.states???
    var s = a[0] == 1 ? 'shift token (then go to state '+a[1]+')' :
        a[0] == 2 ? 'reduce by rule: '+gen.productions._items[a[1]] :
                    'accept' ;
...
parser.init = function parser_init (dict) {
    ...
    this.symbols_ = Jison.SymbolRegistry.deserialize(dict.symbols_);
...
var lalr = generator.beget(lookaheadMixin, lrGeneratorMixin, {
    ...
    afterconstructor: function (grammar, options) {
        ...
        options = options || {};
        this.states = this.canonicalCollection();
        // console.dir({stt: this.states}, {depth: 3});
        this.terms_ = new Map();
        ...
        var newg = this.newg = typal.beget(lookaheadMixin,{
            ...
            nterms_: new Map(),
            DEBUG: false,
            // FIXME: we have amortized this, but it didn't do much!!!
            go_: function gggggo (r, B) {
                // r = r.split(":")[0]; // grab state #
                // B = B.map(function (b) { return b.slice(b.indexOf(":")+1); });
                return this.oldg.go(r, B);
            }
        });
        newg.nonterminals = new Jison.NonterminalForest();
        newg.productions = new Jison.IdFieldItemSet();
    ...
    go: function LALR_go (q, w) {
        for (var i=0;i<w.length;i++) {
            q = this.states.getStateByRef(q).edges[w[i]] || q;
        }
        return q;
    },
    goPath: function LALR_goPath (p, w) {
        ...
        for (var i=0;i<w.length;i++) {
            t = w[i] ? q+":"+w[i] : '';
            if (t)
                this.newg.nterms_.set(t, q);
            path.push(t);
            q = this.states.getStateByRef(q).edges[w[i]] || q;
            this.terms_.set(t, w[i]);
        }
    ...
    buildNewGrammar: function LALR_buildNewGrammar () {
        ...
        // console.dir({ntt: this.nonterminals}, {depth:null});
        this.states.forEach(function build_states_forEach(state, i) {
            // console.dir({ts: self.states, state, i});
            state.forEach(function build_states_state_forEach (item) {
                if (item.dotPosition === 0) {
                    // new symbols are a combination of state and transition symbol
                    const symbol = i+":"+item.production.symbol;
                    self.terms_.set(symbol, item.production.symbol);
                    newg.nterms_.set(symbol, i);

                    if (!newg.nonterminals.hasSeenThisSymbol(symbol))
                        newg.nonterminals.emplaceNew(new Nonterminal(symbol));

                    const pathInfo = self.goPath(i, item.production.handle);
                    const p = new Production(symbol, pathInfo.path, newg.productions.size());
                    // FIXME: we have amortized this, but it didn't do much!!!
                    p.suffixSym = parseInt(p.symbol.split(":")[0], 10);
                    p.suffixHandleSyms = p.handle.map((b) => b.slice(b.indexOf(":")+1));
                    // console.dir({p, pathInfo, symbol});

                    newg.productions.push(p);
                    newg.nonterminals.pushProductionForSymbol(symbol, p);

                    // store the transition that get's 'backed up to' after reduction on path
                    const handle = item.production.handle.join(' ');
                    const goes = self.states.getStateByRef(pathInfo.endState).goes;
                    // console.dir({handle, goes});
                    if (!goes[handle])
                        goes[handle] = [];
    ...
    unionLookaheads: function LALR_unionLookaheads () {
        ...
        states.forEach(function union_states_forEach (i) {
            var state = typeof i === 'number' ? self.states.getStateByRef(i) : i;
            // console.dir({state});
            // console.dir({
            //     s: Object.keys(state),
            ...
            state.reductions.forEach(function union_reduction_forEach (item) {
                ...
                state.goes[item.production.handle.join(' ')].forEach(function reduction_goes_forEach (symbol) {
                    newg.nonterminals.getBySymbol(symbol).follows.forEach(function goes_follows_forEach (symbol) {
                        var terminal = self.terms_.get(symbol);
                        item.follows.add(terminal);
                    });
                });
