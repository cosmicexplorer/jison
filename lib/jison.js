...
var SeenStatesSet = Jison.SeenStatesSet = class SeenStatesSet extends IdFieldItemSet {
    constructor() {
        super();
...
lrGeneratorMixin.SeenStatesSet = Jison.SeenStatesSet;
...
lrGeneratorMixin.canonicalCollection = function canonicalCollection () {
    // FIXME: i think `this.Item` is level 2 of wrapping, not level 3! and `this.productions` is the
    //        first level of wrapping! this is because of the '.id' attribute which is generated for
    //        *both* `item1` and `this.productions[0]` separately!
    // NB: the `[0]` index and `0` `dot` arg below are both intentional!
    // var item1 = new this.Item(this.productions[0], 0, [this.EOF]);
    // console.dir({item1});

    // NB: `topProd` is level 1 of `id`-wrapping sets:
    const topProd = this.productions.first();
    const prodList = new this.Item(topProd, 0, [this.EOF]);
    const topStates = this.IdFieldItemSet.WithElements([prodList]);
    // FIXME: do the list wrapping here via prototypes, not a whole separate class!
    console.dir({topProd, prodList, topStates});

    const firstState = this.closureOperation(topStates);
    const states = this.SeenStatesSet.WithElements([firstState]);
    ...
    while (marked !== states.size()) {
        ...
        console.info('~~~~~~~');
        console.dir({marked, len: states.length});
        console.dir({x: states.item(marked)});
        // TODO: `states.item(marked)` ?????
        let itemSet = states[marked];
        console.info('+++++++');
        // console.dir({itemSet});
        marked++;
