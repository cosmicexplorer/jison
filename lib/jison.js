...
lookaheadMixin.followSets = function followSets () {
            ...
            for (let i = 0; i < production.handle.length; ++i) {
                ...
                const nullable = (function iterate_subsequent_nullable () {
                    for (let j = i + 1; j < production.handle.length; ++j) {
                        ...
                        const nt = nonterminals.getBySymbol(s);
                        if (nt == null) {
                ...
                const qMatches = q === self.nterms_.get(t);
                retProd.push({addTransitive: nullable && qMatches});
            }
            ret.push(retProd);
        }
    ...
    for (const [ntSym, nt] of nonterminals) {
        prevFollows.set(ntSym, Array.from(nt.follows));
    }

    const nextFollows = new Map();
    for (const ntSym of nonterminals.keys()) {
    ...
    while(cont) {
        ...
        productions.forEach(function Follow_prod_forEach (production, k) {
            ...
            const curProd = cachedHandleIteration[k];

            production.handle.forEach(function follow_inner_forEach (t, i) {
                const curTerm = curProd[i];
                if (curTerm == null) {
                    return;
                }
                if (curTerm.addTransitive === false) {
                    return;
                }
        ...
        (function rewrite_amortized_follows () {
            for (const [ntSym, newFollows] of nextFollows) {
                const ntFollows = nonterminals.getBySymbol(ntSym).follows;
