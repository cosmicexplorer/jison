            ...
            for (var i=0,t;t=production.handle[i];++i) {
                ...
                if (i === production.handle.length+1 && bool) {
                    ...
                    if (self.nullable(part) && bool) {
                        set.push.apply(set, pSym.follows);
                    }
                }
                oldcount = curNonterminal.follows.length;
                // (2)FIXME: this is so slow!
                SillySet.union(curNonterminal.follows, set);
                if (oldcount !== curNonterminal.follows.length) {
                    cont = true;
                }
            }
        });
    ...
    // RHS
        ...
        for (var i=0,t;t=symbol[i];++i) {
            if (!this.nonterminals.hasSeenThisSymbol(t)) {
                ...
                SillySet.union(firsts, this.nonterminals.getBySymbol(t).first);
            }
            if (!this.nullable(t))
                break;
        }
...
lookaheadMixin.firstSets = function firstSets () {
    ...
    while(cont) {
        ...
        for (const curNonterminal of nonterminals.values()) {
            ...
            if (firsts.length !== curNonterminal.first.length) {
...
lrGeneratorMixin.parseTable = function parseTable (itemSets) {
    ...
    itemSets.forEach(function (itemSet) {
        ...
        itemSet.forEach(function (item) {
            iii.push(item.markedSymbol);
        });
        rrr.push({iii, e: itemSet.edges, r: itemSet.reductions});
    });
    // console.dir({rrr}, {depth:null});
    var fs = require('fs');
    fs.writeFileSync('wow.json', JSON.stringify(rrr, null, 2));
    throw new Error('ok');
        ...
        itemSet.reductions.forEach(function (item, j) {
            ...
            terminals.forEach(function (stackSymbol) {
                ...
                if (action || action && action.length) {
                    ...
                    if (sol.bydefault) {
                        console.dir({item, k, stackSymbol, sol});
                        self.conflicts++;
                        if (!self.DEBUG) {
